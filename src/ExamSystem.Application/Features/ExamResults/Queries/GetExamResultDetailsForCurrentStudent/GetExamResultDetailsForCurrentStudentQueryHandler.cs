...
{
    public class GetExamResultDetailsForCurrentStudentQueryHandler
        : IRequestHandler<GetExamResultDetailsForCurrentStudentQuery, Result<ExamResultDetailsForCurrentStudent>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetExamResultDetailsForCurrentStudentQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<Result<ExamResultDetailsForCurrentStudent>> Handle(GetExamResultDetailsForCurrentStudentQuery request, CancellationToken cancellationToken)
        {
            ...
            var dto = await examResultQuery
                .ProjectTo<ExamResultDetailsForCurrentStudent>(_mapper.ConfigurationProvider)
            ...
            foreach (var question in dto.Questions)
            {
                ...
                question.StudentOption = new OptionDto
                {
                    OptionId = optionId,
